...
export const getScreenWidthAndHeight = () => (dispatch, getState) => {
    return new Promise((resolve, reject) => {
        const mobile_mql = window.matchMedia("(min-width: 0px) and (max-width: 680px)").matches;
        const tablet_mql = window.matchMedia("(min-width: 681px) and (max-width: 1024px)").matches;
        const desktop_mql = window.matchMedia("(min-width: 1024px)").matches;
        let type = null;
        if(!!mobile_mql){
            type = 'mobile';
        }
